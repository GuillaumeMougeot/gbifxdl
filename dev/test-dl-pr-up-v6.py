...
class AsyncImagePipeline:
    def __init__(
        ...
        self.gpu_image_processors = gpu_image_processor['fn'](device=0, **gpu_image_processor['kwargs']) 
        # if gpu_image_processor is not None:
        #     num_gpus = torch.cuda.device_count()
        #     self.gpu_image_processors = [
        #         ]
    ...
    async def process_image(self, filename: str, processor_id = None) -> bool: 
        ...
        try:
            ...
            # Crop image
            if self.gpu_image_processors is not None and processor_id is not None:
                new_filename = await self.gpu_image_processors.run(img_path)
    ...
    async def processing_consumer(self, i):
        # cropper=Cropper(
        #     "data/classif/mini/fb_xprize_medium.pt",
        #     device=f'cuda:{i%self.max_concurrent_processing}',
        # )
        # cropper=None
        while True:
            url_hash, filename, folder = await self.processing_queue.get()

            # self.logger.info(f"Processing consumer {i} received: {filename}")

            try:
                filename = await self.process_image(filename, processor_id=i)
                # loop = asyncio.get_event_loop()
                # worker = Worker(target=self.process_image, args=(filename, i))
                # self.logger.info(f"Worker {i} starting {filename}")
                # worker.start()
                # self.logger.info(f"Worker {i} started {filename}")
                # filename = await worker.join()

                # self.logger.info(f"Worker {i} processed {filename}")
                if filename is not None:
                    # async with self.metadata_lock:
...
def main():
    downloader = AsyncImagePipeline(
        ...
        gpu_image_processor = dict(
            fn=Cropper,
            kwargs=dict(cropper_model_path="data/classif/mini/fb_xprize_medium.pt")
        )
        # cropper=Cropper(
        #     "data/classif/mini/fb_xprize_medium.pt",
        #     device='cuda:0',
