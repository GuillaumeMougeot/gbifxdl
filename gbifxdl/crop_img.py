...
class Cropper:
    def __init__(
        ...
        if not os.path.exists(cropper_model_path):
            ...
            print(f"Cropping model not found locally, attempting to download it from server at {model_url}")
            cropper_model_path = wget.download(model_url, out=str(model_folder))

        self.cropper = Predictor(
            model = cropper_model_path,
            device=device,
            dtype=torch.float16
        )
    ...
    def run(self, img_path) -> torch.Tensor:
        ...
        try:
            pred = self.cropper.pyramid_predictions(img_path, "", single_scale=True, scale_before=scale_before)
        ...
        if len(pred) == 0:
            ...
            crop_weights = pred.confs * (pred.boxes[:, 2] - pred.boxes[:, 0] > 0) * (pred.boxes[:, 3] - pred.boxes[:, 1] > 0)
