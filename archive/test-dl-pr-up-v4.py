...
class AsyncImagePipeline:
    ...
    async def producer(self):
        ...
        self.download_progress_bar = tqdm(total=total_items, desc="Downloading Images", unit="image")
            ...
            async with asyncssh.connect(**self.sftp_params) as conn:
                async with conn.start_sftp_client() as sftp:
                    ...
                    # await asyncio.gather(*upload_tasks)
                        ...
                        # conn_context = (
                        #     asyncssh.connect(**self.sftp_params)
                        #     if self.sftp_params is not None
                        #     else nullcontext()
                        # )
                        ...
                        #     async with sftp_context as sftp:
                        #         upload_tasks = [
                        #             asyncio.create_task(self.upload_consumer(sftp))
                        #             for _ in range(self.max_concurrent_upload)
                        #         ] if self.sftp_params is not None else []

                        # async with AsyncExitStack() as stack:
                        #     # Optional context for SFTP connection
                        #     sftp_context = nullcontext()
                        #     if self.sftp_params is not None:
                        #         conn = await stack.enter_async_context(asyncssh.connect(**self.sftp_params))
                        #         sftp_context = conn.start_sftp_client()

                        #     async with stack.enter_async_context(sftp_context) as sftp:
                        #         upload_tasks = []
                        #         if sftp:
                        #             upload_tasks = [
                        #                 asyncio.create_task(self.upload_consumer(sftp))
